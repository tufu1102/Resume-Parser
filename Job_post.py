...
def APPLY_JOB():
    ...
    # Get the match percentage using the updated Matching function
    match_percentage = round(Matching(), 2)
    
    # Insert the application data with the match percentage
...
def delete_job():
    ...
    try:
        # Convert job_id to ObjectId and delete the job
        result = JOBS.delete_one({"_id": ObjectId(job_id)})
        
        # Check if the job was successfully deleted
        if result.deleted_count == 1:
            return jsonify({"StatusCode": 200, "Message": "Job deleted successfully"})
        else:
            return jsonify({"StatusCode": 400, "Message": "Failed to delete job"})
